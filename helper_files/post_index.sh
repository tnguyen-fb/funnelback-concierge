...
do
        ...
        curl --connect-timeout 60 --retry 3 --retry-delay 20 'http://localhost:8080/s/search.html?collection='$COLLECTION_NAME'&query=!generate_autoc&profile='$p'&form=auto-completion&view='$CURRENT_VIEW'' -o $SEARCH_HOME/conf/$COLLECTION_NAME/$p/auto-completion.csv || exit 1
